<template>
  <div class="menu-wrapper">
    <template v-for="item in routes">
      <router-link v-if="!item.hidden&&item.children&&item.children.length===1" :to="{ path: item.path+'/'+item.children[0].path, query: item.children[0].query}" :key="item.children[0].name">
