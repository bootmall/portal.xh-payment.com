  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="数据加载中，请稍候..." border fit
      ...
      <el-table-column width="200" align="center" label="操作" class="action-btns" fixed="right">
        <template slot-scope="scope">
          <el-button class="filter-item" size="mini" icon="el-icon-document" v-waves @click="editHandle(scope.row)">修改
          </el-button>
    ...
    >
      <template>
        ...
        <el-radio v-for="(item,key) in channelAccountStatusOptions" :key="key" v-model="statusForm.newStatus"
                  :label="key">{{item}}
        </el-radio>
      </template>
    ...
    <el-dialog title="渠道号编辑" :visible.sync="editVisible" width="40%">
      <el-form :model="editForm" v-loading="editLoading" element-loading-text="数据更新中，请稍候...">
        ...
        <el-form-item :label="'渠道配置-'+item" label-width="160px" v-for="(item,key) in app_secrets_template" :key="key">
          <el-input size="small" type="textarea" :rows="3" v-model="editForm.app_secrets[key]"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button size="small" type="primary" :disabled="submitBtnDisableStatus" @click="editChannelAccount">提交
        </el-button>
      </div>
    </el-dialog>
