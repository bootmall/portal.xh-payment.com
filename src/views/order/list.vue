<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input size="small" class="filter-item" placeholder="收款订单号" v-model="listQuery.orderNo" clearable></el-input>
      <el-input size="small" class="filter-item" placeholder="商户订单号" v-model="listQuery.merchantOrderNo" clearable></el-input>
      <el-input size="small" class="filter-item" placeholder="渠道订单号" v-model="listQuery.channelOrderNo" clearable></el-input>
      <el-input class="filter-item" size="small" v-model="listQuery.merchantNo" placeholder="商户编号" clearable></el-input>
      <el-input size="small" class="filter-item" placeholder="商户账号" v-model="listQuery.merchantUserName" clearable></el-input>
      ...
      <el-button class="filter-item" size="small" type="primary" icon="el-icon-search" @click="handleFilter">搜索
      </el-button>
        ...
        <template slot-scope="scope">
          <el-popover
            ...
            <span class="link-type" slot="reference"
                  @click="showDetail(scope.row)">{{scope.row.notify_status_str}}</span>
          </el-popover>
        </template>
      </el-table-column>
    ...
    <el-dialog title="设置为成功" :visible.sync="setSuccessVisible" width="50%">
      <el-form :model="setSuccessForm">
        <el-form-item label-width="120px" label="渠道订单号：">
          <el-input type="text" :rows="3" style="width: 280px" v-model="setSuccessForm.channel_order_no"></el-input>
        </el-form-item>
        <el-form-item label-width="120px" label="最终订单号：">
          <el-input type="text" :rows="3" style="width: 280px"
                    v-model="setSuccessForm.final_channel_order_no"></el-input>
        </el-form-item>
        <el-form-item label-width="120px" label="备注：">
          <el-input type="textarea" :rows="3" style="width: 280px" v-model="setSuccessForm.bak">
          </el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="setSuccessVisible = false">取 消</el-button>
        <el-button type="primary" @click="setSuccess">提交</el-button>
      </div>
    </el-dialog>
    ...
    export default {
        ...
        data() {
            return {
                ...
                setSuccessForm: setSuccessFormFields,
                summery: {
                    amount: 0
                },
                notify_url: null,
        ...
        methods: {
            ...
            setSuccess() {
                ...
                self.$confirm('此操作将订单设置为成功, 是否继续?', '提示', {
                    ...
                    axios.post('/admin/order/set-success', self.setSuccessForm).then(
                        res => {
                            this.setSuccessVisible = false
                            if (res.code != 0) {
                                self.$message.error({message: res.message})
                            } else {
                                self.$message.success({message: res.message})
