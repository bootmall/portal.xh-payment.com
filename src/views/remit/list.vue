<template>
    <div class="app-container calendar-list-container">
        <div class="filter-container">
            ...
            <el-input @keyup.enter.native="handleFilter" size="small" style="width: 200px;" class="filter-item" placeholder="卡号" v-model="listQuery.bankNo"></el-input>
            ...
            <el-input class="filter-item" size="small" style="width: 200px;" @change.native="checkNumber()" v-model="listQuery.minMoney" placeholder="金额"></el-input>
            -
            <el-input class="filter-item" size="small" style="width: 200px;" @change.native="checkNumber()" v-model="listQuery.maxMoney"></el-input>
...
" @click="setChecked(scope.row.id)" v-waves>审核
                    ...
                    <el-button class="filter-item" icon="el-icon-refresh" size="mini" v-if="[30,40,50,60,-10,-20].indexOf(parseInt(scope.row.status)) !== -1" @click="syncStatus(scope.row.id)" v-waves>同步</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      createTrack() {
        self = this;
        axios.post('/admin/track/add', {parentId: self.trackForm.id, parentType: 'remit', type: self.trackForm.type, upload: self.trackForm.upload, note: self.trackForm.note}).then(
          res => {
            if (res.code != 0) {
              self.$message.error({message: res.message})
            } else {
