<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" size="small" type="success" v-waves icon="search"
                 @click="handleShowUpdateStatus(1,'')">批量审核通过
      </el-button>
      <el-button class="filter-item" size="small" type="danger" v-waves icon="search"
                 @click="handleShowUpdateStatus(2,'')">批量拒绝并退款
      </el-button>
    </div>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="数据加载中，请稍候..." border fit
      ...
      <el-table-column fixed="right" width="160" align="center" label="操作" class-name="op-column">
        <template slot-scope="scope">
          <el-button class="filter-item" size="small" type="success" v-waves icon="search"
                     @click="handleShowUpdateStatus(1,scope.row.id)">通过
          </el-button>
          <el-button class="filter-item" size="small" type="danger" v-waves icon="search"
                     @click="handleShowUpdateStatus(2,scope.row.id)">拒绝
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            handleUpdateStatus() {
                ...
                if (!self.checkForm.remitIdList || self.checkForm.remitIdList.length == 0) {
                    this.$alert('订单选择失败,请从新选择订单', '失败提示', {
